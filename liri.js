        ...
        case "concert-this":
            axios.get("https://rest.bandsintown.com/artists/" + input + "/events?app_id=codingbootcamp")
                .then(function(response) {
                    ...
                    let text = [`The venue name is: ${response.data[0].venue.name}`,
                        `The venue location is: ${response.data[0].venue.city}, ${response.data[0].venue.region}, ${response.data[0].venue.country}`,
                        `The date of the event is: ${moment(response.data[0].datetime).format("MM/DD/YYYY")}`
                    ]
        ...
        case "spotify-this-song":
            ...
            if (process.argv.length == 3) {
                spotify.search({ type: 'track', query: "The Sign Ace of Base" }, function(err, data) {
                    ...
                    console.log("---------------------------------------------------------------")
                    console.log(`The name of the artist is: ${data.tracks.items[0].artists[0].name}`);
                    console.log(`The name of the song is: ${data.tracks.items[0].name}`);
                    console.log(`The preview link to the song is: ${data.tracks.items[0].preview_url}`);
                    console.log(`The album name of this song is: ${data.tracks.items[0].album.name}`);
                    console.log("---------------------------------------------------------------")

                    let text = [`The name of the artist is: ${data.tracks.items[0].artists[0].name}`,
            ...
            if (process.argv.length == 3) {
                ...
                axios.get("http://www.omdbapi.com/?t=" + input + "&y=&plot=short&apikey=trilogy")
                    .then(function(response) {
                        ...
                        let text = [`The title is: ${response.data.Title}`,
